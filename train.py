...
def train_model_5cv():
    ...
    for fold, (train_idx, valid_idx) in enumerate(kfold.split(dataset)):
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=config.scheduler_factor,
                                                               patience=config.scheduler_patience, threshold=0.0001,
        ...
        for epoch in tqdm(range(config.n_epoch), position=0, leave=True):
            ...
            wandb.log({f"{fold} fold train": {"loss": epoch_train_loss}, f"{fold} fold val": {"loss": epoch_valid_loss},
